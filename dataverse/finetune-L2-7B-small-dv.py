# This script uses a small subset of DV data to fine-tune LLAMA2
# (S.M.Iacus 2024)
...
new_model = "siacus/llama-2-7b-small-dv" # you need to change 'siacus' to your HF account

dataset = load_dataset(cap_dataset, split={'train': 'train', 'test': 'test'})

# Shuffle and sample 5,000 rows from the train split
train_dataset = dataset['train'].shuffle(seed=123).select(range(5000))

# Shuffle and sample 5,000 rows from the test split
test_dataset = dataset['test'].shuffle(seed=123).select(range(5000))
